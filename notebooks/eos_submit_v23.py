# === CELL 2 (V23: weight variation + BirdNET test) ===
solutions = {
 'type_add' :'direct',
 'Models'   : [
...
if 'Model_4' in _ensemble_models:
    ...
    # V23: Optimized ratios — more SED+BirdNET diversity, less ProtoSSM dominance
    if rank_birdnet is not None:
        ...
        print("Executing 2-way rank blend (55% Proto / 45% SED)...")
    ...
    # ?? Gate 3: SED spike preservation (V23: threshold 0.92→0.90, more permissive)
    # Brief high-confidence SED detections that ProtoSSM missed
    sed_only = (rank_sed > 0.90) & (rank_proto < 0.80) & (~fake_only) & (~proto_cont)
    pred = np.where(sed_only, (1.0 - 0.12) * pred + 0.12 * rank_sed, pred)
    # ?? Gate 3b: BirdNET spike preservation (V23: threshold 0.92→0.90)
    ...
    if rank_birdnet is not None:
        bn_only = (rank_birdnet > 0.90) & (rank_proto < 0.75) & (rank_sed < 0.80) & (~fake_only) & (~proto_cont) & (~sed_only)
...
print("Taxon-aware temporal smoothing applied (V23: weight variation test)")
...
if tax is not None:
    ...
    print("Taxonomic smoothing applied (V23: weight variation test)")
else:
