# === CELL 2 (V26: BirdNET fix + OOF post-proc) ===
solutions = {
 'type_add' :'direct',
 'Models'   : [
  # Model_2 RETIRÉ, BirdNET fix dans Model_4
...
if 'Model_2' in _ensemble_models or \
   'task' in solutions and 'run Model_2_SED once' in solutions['task']:
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", "/kaggle/input/datasets/hellodave2035/birdclef-perch-models/onnxruntime-1.24.4-cp312-cp312-manylinux_2_27_x86_64.manylinux_2_28_x86_64.whl"])
    ...
    if MODE == "train":
        import subprocess, sys
        subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", "timm", "torchaudio", "onnxscript", "onnx"])
...
if 'Model_3' in _ensemble_models:
    ...
    ort_whl = "/kaggle/input/datasets/hellodave2035/birdclef-perch-models/onnxruntime-1.24.4-cp312-cp312-manylinux_2_27_x86_64.manylinux_2_28_x86_64.whl"
    ...
    # ? ONNX Load
    ONNX_PERCH_PATH = Path("/kaggle/input/datasets/hellodave2035/birdclef-perch-models/perch_v2.onnx")
    ...
    def build_oof_base_prior(scores_full_raw, meta_full, sc_clean, Y_SC, n_splits=5, verbose=True):
        ...
        Y_ALIGNED = Y_SC[aligned_indices]  # ?????? 708 ?????
...
if 'Model_4' in _ensemble_models:
    ...
    def find_wheel(pattern):
        for p in INPUT_ROOT.rglob(pattern):
            return p
        print(f"Wheel not found: {pattern}"); return None  # non-fatal
    ...
    def _find_birdnet_model():
        # V26 FIX v3: Test exact known paths with debug
        direct_paths = [
            Path("/kaggle/input/models/shadiakiki1/birdnet-analyzer/tflite/birdnet_global_6k_v2.4_model_fp32-1"),
            Path("/kaggle/input/models/shadiakiki1/birdnet-analyzer/tflite/birdnet_global_6k_v2.4_model_fp32-1/V3"),
        ...
        parent = Path("/kaggle/input/models/shadiakiki1/birdnet-analyzer/tflite/birdnet_global_6k_v2.4_model_fp32-1")
        print(f"  [DEBUG] parent exists: {parent.exists()}")
        if parent.exists():
            print(f"  [DEBUG] parent contents: {sorted(parent.iterdir())}")
        for dp in direct_paths:
            print(f"  [DEBUG] checking: {dp} (exists={dp.exists()})")
        
        for direct_base in direct_paths:
            if not direct_base.exists():
                continue
            for pat in ["*.tflite", "**/*.tflite"]:
                hits = sorted(direct_base.rglob(pat))
                if hits:
                    print(f"  Found BirdNET model: {hits[0]}")
                    return hits[0]
        # Fallback: recursive search in /kaggle/input
        for pat in ["**/birdnet_global_6k_v2.4_model_fp32*.tflite",
            ...
            if hits:
                print(f"  Found BirdNET model (glob): {hits[0]}")
                return hits[0]
        return None
    ...
    try:
        ...
        if _bn_active:
            ...
            print("Executing 3-way rank blend (50% Proto / 25% SED / 25% BirdNET)...")
        else:
            rank_birdnet = None
    ...
    # V26: Optimized 3-way blend with BirdNET active
    if rank_birdnet is not None:
        print("Executing 3-way rank blend (45% Proto / 30% SED / 25% BirdNET)...")
        pred = (rank_proto * 0.45) + (rank_sed * 0.30) + (rank_birdnet * 0.25)
    else:
        print("Executing 2-way rank blend (60% Proto / 40% SED)...")
    ...
    # ?? Gate 3: SED spike preservation (V21: lowered threshold 0.95→0.92)
    # Brief high-confidence SED detections that ProtoSSM missed
    sed_only = (rank_sed > 0.92) & (rank_proto < 0.80) & (~fake_only) & (~proto_cont)
    pred = np.where(sed_only, (1.0 - 0.12) * pred + 0.12 * rank_sed, pred)
    # ?? Gate 3b: BirdNET spike preservation (V21: lowered threshold 0.95→0.92)
    ...
    if rank_birdnet is not None:
        bn_only = (rank_birdnet > 0.92) & (rank_proto < 0.75) & (rank_sed < 0.80) & (~fake_only) & (~proto_cont) & (~sed_only)
...
def rank_1_add2():
    ...
    subm       = SED.copy()
    subm[cols] = pred.astype("float32") #;submission.to_csv('submission.csv',index=False)
    return subm
...
# === CELL 15 ===
# V13: Taxon-aware temporal smoothing
...
print(f"Taxon smoothing: {n_tex} texture (alpha=0.15), {n_evt} event (alpha=0.05)")

def taxon_smooth(df, texture_cols, event_cols, alpha_texture=0.25, alpha_event=0.08):
...
print("Taxon-aware temporal smoothing applied (V26: BirdNET fix)")
submission = submission.set_index("row_id")


# === CELL 16 ===
# V14: Per-species Platt calibration (FIXED: logit->prob conversion)
...
if oof_loaded:
    ...
    if labels_csv.exists():
        ...
        for _s_idx in range(_N_CLASSES):
            ...
            _lr = LogisticRegression(C=10.0, max_iter=1000, class_weight="balanced")  # V22: C=10 (was 100, more regularization per OOF)
...
# === CELL 17 ===
# V11: Taxonomic smoothing (genus + class level)
...
if tax is not None:
    ...
    alpha_genus = 0.15  # V22: OOF-optimized (was 0.10)
    alpha_class = 0.02  # V22: OOF-optimized (was 0.03)
    ...
    print("Taxonomic smoothing applied (V26: BirdNET fix)")
else:
...
print(f'Sample: {len(sample_sub)} | Submission: {len(sub)}')
if len(sub) != len(sample_sub):
    print('DRY-RUN: overriding')
    sample_sub.to_csv('/kaggle/working/submission.csv', index=False)
sub2 = pd.read_csv('/kaggle/working/submission.csv')
assert sub2.shape == sample_sub.shape
print('VALID: submission.csv ready')
